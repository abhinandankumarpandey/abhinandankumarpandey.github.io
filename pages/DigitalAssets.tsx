  ...
  return (
    <div className="pt-40 px-6 max-w-7xl mx-auto pb-40">
      <div className="mb-24 text-center">
        <h1 className="text-7xl md:text-[10rem] font-bold tracking-tighter mb-8">THE <span className="text-gradient-liquid">VAULT.</span></h1>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-12">
          {items.map((asset, idx) => (
            <div
              key={idx}
              className={`group glass rounded-[3rem] p-4 flex flex-col md:flex-row gap-8 transition-all duration-500 border relative overflow-hidden ${highlightIndex === idx
            ...
            >
              {/* Image Section */}
              <div className="w-full md:w-1/3 md:max-w-xs aspect-[4/5] md:aspect-square rounded-[2.2rem] overflow-hidden relative flex-shrink-0">
                <img
              ...
              <div className="flex-1 flex flex-col justify-center px-4 md:px-8 py-4">
                <div>
                  <h3 className="text-3xl md:text-5xl font-bold mb-6 tracking-tight uppercase group-hover:text-indigo-400 transition-colors">{asset.title}</h3>
                  <p className="text-zinc-500 mb-8 leading-relaxed text-lg max-w-2xl">{asset.description}</p>
                </div>

                <div className="mt-auto flex flex-wrap items-center justify-between gap-6 pt-6 border-t border-white/5">
                  <span className={`text-4xl font-black font-mono tracking-tighter transition-colors ${highlightIndex === idx ? 'text-white' : 'text-zinc-300'}`}>{asset.price}</span>
                  <a
                    href={asset.buy_link || '#'}
                    target="_blank"
                    rel="noopener noreferrer"
                    className={`px-10 py-4 font-bold rounded-full hover:scale-105 transition-all duration-300 uppercase tracking-widest text-sm ${!asset.buy_link ? 'opacity-50 cursor-not-allowed bg-zinc-800 text-zinc-500' :
